...
export class Database {
  ...
  async getSyncState(): Promise<SyncState | null> {
    // 暂时只支持 SQLite，PostgreSQL 需要适配层
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getSyncState is only supported for SQLite');
    }
  ...
  async updateSyncState(lastBlock: number, startBlock?: number): Promise<void> {
    // 暂时只支持 SQLite，PostgreSQL 需要适配层
    if (DB_TYPE !== 'sqlite') {
      throw new Error('updateSyncState is only supported for SQLite');
    }
  ...
  async insertTransfer(data: TransferData): Promise<void> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('insertTransfer is only supported for SQLite');
    }
  ...
  async getTransfersWithFilters(params: TransfersFilterParams): Promise<{ transfers: any[]; total: number }> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getTransfersWithFilters is only supported for SQLite');
    }
  ...
  async getLatestTransfers(limit: number = 20): Promise<any[]> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getLatestTransfers is only supported for SQLite');
    }
  ...
  async getAddressTransferSummary(address: string): Promise<TransferSummary> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getAddressTransferSummary is only supported for SQLite');
    }
  ...
  async getAddressTransferGraph(address: string, limit: number = 20): Promise<TransferGraph> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getAddressTransferGraph is only supported for SQLite');
    }
  ...
  async getDailyTransferSummary(date: string): Promise<DailyTransferSummary | null> {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getDailyTransferSummary is only supported for SQLite');
    }
  ...
  async updateLastProcessedSwapId(swapId: number): Promise<void> {
    ...
    // 注意：此方法仅支持 SQLite，PostgreSQL 需要不同的实现
    if (DB_TYPE !== 'sqlite') {
      throw new Error('updateLastProcessedSwapId is only supported for SQLite');
    }
  ...
  async getBuyVolumeTop(limit: number = 50, startTime?: number, endTime?: number) {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getBuyVolumeTop is only supported for SQLite');
    }
  ...
  async getSellVolumeTop(limit: number = 50, startTime?: number, endTime?: number) {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getSellVolumeTop is only supported for SQLite');
    }
  ...
  async getBuyCountTop(limit: number = 20, startTime?: number, endTime?: number) {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getBuyCountTop is only supported for SQLite');
    }
  ...
  async getSellCountTop(limit: number = 20, startTime?: number, endTime?: number) {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getSellCountTop is only supported for SQLite');
    }
  ...
  async getTradeCountTop(limit: number = 20, startTime?: number, endTime?: number) {
    if (DB_TYPE !== 'sqlite') {
      throw new Error('getTradeCountTop is only supported for SQLite');
    }
